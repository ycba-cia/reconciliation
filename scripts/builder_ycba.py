...
def make_qualified_group_actor(a,aqa,actor):
	...
	if not hasattr(who, 'formed_by'):
		b = model.Formation()
		who.formed_by = b
	who.formed_by.carried_out_by = actor
	return who
...
if config1 == "test":
	sql = "select local_identifier, xml from metadata_record where local_identifier in (82154,258,872) and status != 'deleted' order by cast(local_identifier as signed) asc"
	#sql = ""
else:
...
if config1 == "test":
	...
	sql = "SELECT local_identifier,set_spec FROM record_set_map where local_identifier in (82154,258,872) order by cast(local_identifier as signed) asc"
else:
			...
			if exid:
				...
				if e_url:
					...
					hp.identified_by = vocab.PrimaryName(
						content=f"View the exhibition page on the Yale Center for British Art website")
					hp.format = "text/html"
